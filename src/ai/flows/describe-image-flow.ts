...
Scene Description:`,
  ...
  const describeImageFlowInternal_local_action = ai.defineFlow(
    {
      name: 'describeImageFlow_local_action', // Unique name
    ...
    async (flowInput) => {
      ...
      try {
        ...
        if (!output?.description) {
          ...
          if (flowInput.language === 'es') errorMsg = "Error: La IA no pudo generar una descripción.";
          else if (flowInput.language === 'pt') errorMsg = "Erro: A IA não conseguiu gerar uma descrição.";
  ...
  return describeImageFlowInternal_local_action(input);
}
